...
def read_variables_from_file(file_path):
    ...
    return variables
variables = read_variables_from_file("config4.txt")
...
if positives <= round(len(y_positives)/len(y_train),2):
    oversampling = 0
    undersampling = 0
    print("It is not possible to do undersampling/oversampling because the original proportion of positives is bigger than the new intended proportion of positives.\n")
...
def get_user_response(prompt):
    while True:
        ...
        if response in ['y', 'n']:
            ...
            print("Error: Answer not recognized. Please enter 'y' or 'n'.")

if model_selection == "lr":
    save_model = get_user_response("Would you like to use this logistic regression (y/n)? ")
    
if  model_selection == "dt":
    save_model = get_user_response("Would you like to use this decision tree (y/n)? ")
...
if save_model == "n":
    ...
    print("Continuing with the script as per the user's choice.")
