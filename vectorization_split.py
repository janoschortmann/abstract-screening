...
print("The {} citations from the sample have been loaded.\n".format(len(full_texts_sample)))
...
unigrams = np.array(pd.read_csv(pathfile_unigrams,header=None))
unigrams = unigrams[:,0].reshape((len(unigrams),1))

print("{} UNI-grams have been loaded.\n".format(len(unigrams)))


#Inicializar vectorizacion
print("Vectorization is going to start.")

vec = np.zeros((len(full_texts_sample),len(unigrams)))

for i in range(len(full_texts_sample)):
    for j in range(len(unigrams)):
        if unigrams[j,0] in full_texts_sample[i,0]:
            vec[i,j] = full_texts_sample[i,0].count(unigrams[j,0])
            
vec_citations = np.zeros((len(full_texts),len(unigrams)))

for i in range(len(full_texts)):
    for j in range(len(unigrams)):
        if unigrams[j,0] in full_texts[i,0]:
            vec_citations[i,j] = full_texts[i,0].count(unigrams[j,0])
